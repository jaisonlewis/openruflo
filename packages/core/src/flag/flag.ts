...
// openruflo aliases - prefer OPENRUFLO_* env vars, fall back to OPENCODE_* for compatibility
export const RufloFlag = {
  get CONFIG_DIR() {
    return process.env["OPENRUFLO_CONFIG_DIR"] ?? process.env["OPENCODE_CONFIG_DIR"]
  },
  get CONFIG() {
    return process.env["OPENRUFLO_CONFIG"] ?? process.env["OPENCODE_CONFIG"]
  },
  DISABLE_AUTOUPDATE:
    truthy("OPENRUFLO_DISABLE_AUTOUPDATE") || truthy("OPENCODE_DISABLE_AUTOUPDATE"),
  BIN_PATH: process.env["OPENRUFLO_BIN_PATH"] ?? process.env["OPENCODE_BIN_PATH"],
}

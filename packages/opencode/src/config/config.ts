...
function globalConfigFile() {
  const candidates = ["openruflo.jsonc", "openruflo.json", "config.json"].map((file) =>
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        data.$schema = "https://openruflo.ai/config.json"
        const updated = text.replace(/^\s*\{/, '{\n  "$schema": "https://openruflo.ai/config.json",')
    ...
    const loadGlobal = Effect.fnUntraced(function* () {
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "openruflo.json")))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "openruflo.jsonc")))
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result["$schema"] = "https://openruflo.ai/config.json"
      ...
      function* (ctx: InstanceContext) {
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            if (!remoteConfig.$schema) remoteConfig.$schema = "https://openruflo.ai/config.json"
        ...
        if (!Flag.OPENCODE_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("openruflo", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        for (const dir of directories) {
          if (dir.endsWith(".openruflo") || dir === Flag.OPENCODE_CONFIG_DIR) {
            for (const file of ["openruflo.json", "openruflo.jsonc"]) {

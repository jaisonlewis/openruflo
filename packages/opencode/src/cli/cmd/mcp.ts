...
import { resolveBuiltinMcpServers } from "../../mcp/builtin"
...
function configuredServers(config: Config.Info) {
  ...
  const merged = { ...resolveBuiltinMcpServers(userMcp), ...userMcp }
  return Object.entries(merged).filter((entry): entry is [string, McpConfigured] => isMcpConfigured(entry[1]))
}
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .openruflo/ subdirectory too)
  const candidates = [path.join(baseDir, "openruflo.json"), path.join(baseDir, "openruflo.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".openruflo", "openruflo.json"), path.join(baseDir, ".openruflo", "openruflo.jsonc"))
  }

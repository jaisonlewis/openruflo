...
import { resolveBuiltinMcpServers } from "./builtin"
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make<State>(
      Effect.fn("MCP.state")(function* () {
        ...
        // Merge built-in MCP servers (user config keys take precedence)
        const userMcp = cfg.mcp ?? {}
        const config = { ...resolveBuiltinMcpServers(userMcp), ...userMcp }
        const s: State = {
          status: {},
          clients: {},
          defs: {},
    ...
    const status = Effect.fn("MCP.status")(function* () {
      ...
      const cfg = yield* cfgSvc.get()
      const userMcp2 = cfg.mcp ?? {}
      const config = { ...resolveBuiltinMcpServers(userMcp2), ...userMcp2 }

...
// Use fs.rmSync on Windows where `rm -rf` may fail on locked exes
try {
  fs.rmSync(path.join(dir, "dist"), { recursive: true, force: true })
} catch { /* ignore — dist may not exist */ }
...
for (const item of targets) {
  ...
  await Bun.build({
    ...
    compile: {
      ...
      outfile: `dist/${name}/bin/openruflo`,
      execArgv: [`--user-agent=openruflo/${Script.version}`, "--use-system-ca", "--"],
      windows: {},
    },
  ...
  if (item.os === process.platform && item.arch === process.arch && !item.abi) {
    const binaryPath = `dist/${name}/bin/openruflo`
